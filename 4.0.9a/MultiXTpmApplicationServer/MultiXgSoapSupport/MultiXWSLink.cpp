...
//	m_pSoap->imode	|=	SOAP_IO_KEEPALIVE;
  ...
  m_pSoap->fpoll = NULL;
	...
	m_phttp_parse_header	=	m_pSoap->fparsehdr;
	m_pSoap->fparsehdr	=	gSoapHttpParseHeader;
	m_pSoap->user	=	this;
}
...
std::string	&CMultiXWSLink::VirtualPath()
{

	if(m_VirtualPath.empty())
	{
		char	*Ptr	=	strchr(m_pSoap->path, '?');
		if(Ptr	==	NULL)
			m_VirtualPath	=	m_pSoap->path+1;
		else
		{
			char Query[SOAP_TAGLEN+100];
			memcpy(Query,m_pSoap->path+1,SOAP_TAGLEN);
			Ptr	=	strchr(Query, '?');
			*Ptr	=	0;
			m_VirtualPath	=	Query;
		}
...
void	CMultiXWSLink::ParseQueryString()
{

  char Query[SOAP_TAGLEN];
	m_HTTPQueryParams.clear();

	char	*Ptr	=	strchr(m_pSoap->path, '?');
	if(Ptr	==	NULL)
		return;
	memcpy(Query,Ptr,SOAP_TAGLEN);
	Ptr	=	strchr(Query, '?');
	while(Ptr)
	{
		char	*Key	=	GetNextQueryKey(&Ptr);
		char	*Val	=	GetNextQueryKey(&Ptr);
		if(Key	!=	NULL)
			if(Val	!=	NULL)
				m_HTTPQueryParams[Key]	=	Val;
			else
				m_HTTPQueryParams[Key]	=	"";
	}
}

int	CMultiXWSLink::GetHTTPHeaderIntParam(std::string	ParamName,int	DefaultValue)
{
	return	CMultiX::GetIntParam(m_HTTPHeaders,ParamName,DefaultValue);
}
std::string	CMultiXWSLink::GetHTTPHeaderStringParam(std::string	ParamName,std::string	DefaultValue)
{
	return	CMultiX::GetStringParam(m_HTTPHeaders,ParamName,DefaultValue);
}
int	CMultiXWSLink::GetHTTPQueryIntParam(std::string	ParamName,int	DefaultValue)
{
	return	CMultiX::GetIntParam(m_HTTPQueryParams,ParamName,DefaultValue);
}
std::string	CMultiXWSLink::GetHTTPQueryStringParam(std::string	ParamName,std::string	DefaultValue)
{
	return	CMultiX::GetStringParam(m_HTTPQueryParams,ParamName,DefaultValue);
}

char	*CMultiXWSLink::GetNextQueryKey(char	**Query)
{ 
	char *t = *Query;
  if (t && *t)
  { 
		*Query = (char*)DecodeSoapString(t, strlen(t), t + 1);
    return t;
  }
  return *Query = NULL;
}
char	*CMultiXWSLink::GetNextQueryVal(char	**Query)
{ 
	char *t = *Query;
  if (t && *t == '=')
  { 
...
size_t	CMultiXWSLink::EncodeSoapString(const char *Source, char *Dest, size_t len)
{ 
	register int c;
...
const char* CMultiXWSLink::DecodeSoapString(char *Dest, size_t len, const char *Source)
{ 
	const char *s;
  char *t;
  for (s = Source; *s; s++)
...
int	CMultiXWSLink::gSoapHttpParseHeader(soap *S, const char *key, const char *val)
{
	CMultiXWSLink	*Link	=	(CMultiXWSLink	*)S->user;
	return	Link->OngSoapHttpParseHeader(key,val);
}
...
int	CMultiXWSLink::OngSoapHttpParseHeader(const char *Key, const char *Val)
{
	if(Key	!=	NULL)
		if(Val	!=	NULL)
			m_HTTPHeaders[Key]	=	Val;
		else
			m_HTTPHeaders[Key]	=	"";

	return	m_phttp_parse_header(m_pSoap,Key,Val);
}

void	CMultiXWSLink::EnableDebug(int	DebugLevel)
{
...
{
	if(m_DebugLevel	>	0)
		DebugPrint(m_DebugLevel,"Data Received Size=%d Data=\n%s\n",Buf.Length(),Buf.DataPtr());
...
{
	const   char	*ContinueStr	=	"HTTP/1.1 100 Continue";	//	Length	=	21
	...
	if(m_bSendingContinue)
	{
		...
		if(OutBuf().Length()	>=	25	/* size of continue std::string + crlf +crlf */)
		{
			...
			{
				if(m_DebugLevel	>	0)
					DebugPrint(m_DebugLevel,"Sending Buffer Size=%d Data=\n%s\n",OutBuf().Length(),OutBuf().DataPtr());
				Send(OutBuf(),true);
				if(m_pLastBufferSent)
					m_pLastBufferSent->ReturnBuffer();
...
bool	CMultiXWSLink::CheckAndSendGetFile(std::string RootDir)
{
	std::string	URL	=	m_pSoap->path;
  ...
  if (!fd) 
		return false;
  m_pSoap->http_content = "text/html"; // HTTP header with text/xml content 
	if(FileName.find(".js")	!=	FileName.npos)
		m_pSoap->http_content	=	"application/x-javascript";
	else if(FileName.find(".css")	!=	FileName.npos)
		m_pSoap->http_content	=	"text/css";
