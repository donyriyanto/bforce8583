...
#else
int	DimeServerService(struct soap *S)
{
	soap_set_namespaces(S,namespaces);

  // set DIME callbacks
  S->fdimereadopen = dime_read_open;
  S->fdimereadclose = dime_read_close;
  S->fdimeread = dime_read;
  S->fdimewriteopen = dime_write_open;
  S->fdimewriteclose = dime_write_close;
  S->fdimewrite = dime_write;
	return	soap_serve(S);
