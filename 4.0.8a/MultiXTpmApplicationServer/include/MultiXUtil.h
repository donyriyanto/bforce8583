...
public:
	...
	*/
	...
	it copies the data to the end of the buffer. During the entire process, the buffer is locked, and
	only the calling thread has write access to the buffer. The size of the appended data includes the terminating null.
	
	\param	StringData a pointer to the null terminated C string. If it is empty, a null byte is appended. If it is NULL, no data is appended.
	\return	a read only pointer to the new underlying buffer.
	*/
	const	char_t	*AppendString(const	char_t	*StringData);
...
private:
	...
	bool	IsEmpty();
	EXPORTABLE_STL::vector<ObjectType	*>	&Objects();
	size_t	Size();
	void	Lock();
	void	Unlock();
	void	DeleteAll();
private:
	EXPORTABLE_STL::vector<ObjectType	*>	m_Objects;
...
private:
	EXPORTABLE_STL::vector<CMultiXBuffer *>	m_Buffers;
};
