...
void CMultiXLogger::ReportError(std::string Msg)
{
	...
	if(m_bSendMailOnError	&&	m_MailerMsgCode	>	0	&&	m_MailFrom.length()	>	0	&&	m_MailTo.length()	>	0)
	{
		CMultiXProcess	*pProcess	=	m_pApp->FindProcess(m_pApp->TpmProcID());
		if(pProcess)
		{
			char	Subject[1000];
			sprintf(Subject,"Error Report from MultiX Process %d (%s)",m_pApp->ProcessID(),m_pApp->ProcessClass().c_str());
			CMultiXBuffer	*pBuf	=	m_pApp->AllocateBuffer();
			pBuf->AppendString(m_MailFrom.c_str());
			pBuf->AppendString(m_MailTo.c_str());
			pBuf->AppendString(Subject);
			pBuf->AppendString(Msg.c_str());
...
void CMultiXLogger::ReportInfo(std::string Msg)
{
	const	char	*Ptr	=	Msg.c_str();
