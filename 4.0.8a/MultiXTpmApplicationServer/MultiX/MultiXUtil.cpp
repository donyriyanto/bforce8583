...
const	char_t	*CMultiXBuffer::AppendString(const	char_t	*Data)
{
	if(Data	!=	NULL)
	{
		return	AppendData(Data,(int)strlen(Data)+1);
	}
	return	DataPtr();
}
...
template	<class	ObjectType,class	ObjectOwner>
EXPORTABLE_STL::vector<ObjectType	*>	&CMultiXVector<ObjectType,ObjectOwner>::Objects()
{
	return	m_Objects;
}
template	<class	ObjectType,class	ObjectOwner>
