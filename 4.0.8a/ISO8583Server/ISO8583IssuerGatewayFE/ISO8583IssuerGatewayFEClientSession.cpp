/*!	\file	*/
// ISO8583IssuerGatewayFEClientSession.cpp: implementation of the CISO8583IssuerGatewayFEClientSession class.
//
//////////////////////////////////////////////////////////////////////
/*
...
CISO8583IssuerGatewayFEClientSession::CISO8583IssuerGatewayFEClientSession(CISO8583IssuerGatewayFEApp	&Owner)	:
	CMultiXSession(Owner)
{
}

//! just make sure the m_pLastTpmMsg deleted, if not , we have a bug
CISO8583IssuerGatewayFEClientSession::~CISO8583IssuerGatewayFEClientSession()
{
//	DebugPrint("Deleted\n");
}
...
*/
bool CISO8583IssuerGatewayFEClientSession::CanDelete()
{
	return	true;
}
...
/*!
	...
	to no link object.
*/
void CISO8583IssuerGatewayFEClientSession::OnLinkDisconnected()
{
...
*/
void CISO8583IssuerGatewayFEClientSession::OnLinkConnected(CISO8583IssuerGatewayFELink *pLink)
{
...
\return	true if the message is a valid ISO 8583 message or false otherwise.

	The associated link notifies us of a newly received message.
*/
bool CISO8583IssuerGatewayFEClientSession::OnNewPartnerMsg(CISO8583Msg &ISOMsg)
{
	if(!Send(ISOMsg.MTI(),ISOMsg.ISOBuffer(),ISOMsg.ISOBufferSize(),CMultiXAppMsg::FlagNotifyAll))
	{
		Throw();
	}
...
*/
void	CISO8583IssuerGatewayFEClientSession::OnSendMsgCompleted(CMultiXAppMsg &OriginalMsg)
{
	DebugPrint(2,"Send Msg Completed\n");
		CISO8583IssuerGatewayFELink	*pLink	=	GetLink();
	if(pLink)
...
*/
void	CISO8583IssuerGatewayFEClientSession::OnSendMsgFailed(CMultiXAppMsg &OriginalMsg)
{
...
/*!
	...
	was not processed succefuly by the server application.
*/
void	CISO8583IssuerGatewayFEClientSession::OnSendMsgTimedout(CMultiXAppMsg &OriginalMsg)
{
...
*/
void	CISO8583IssuerGatewayFEClientSession::OnDataReplyReceived(CMultiXAppMsg &ReplyMsg,CMultiXAppMsg &OriginalMsg)
{
	...
	if(ReplyMsg.AppDataSize()	==	0)	//	an empty reply from the server application
	{
		...
		{
			CISO8583Msg	ISOMsg;
			ISOMsg.FromISO((byte_t	*)OriginalMsg.AppData(),OriginalMsg.AppDataSize());
			ISOMsg.SetMTI(ISOMsg.MTI()+10);
			if(ISOMsg.Version()	==	CISO8583Msg::ISO8583_1_1987)
				ISOMsg.SetActionCode("90");
			else
				ISOMsg.SetActionCode("904");
			ISOMsg.ToISO();
			CMultiXBuffer	*pBuf	=	Owner()->AllocateBuffer((char	*)ISOMsg.ISOBuffer(),ISOMsg.ISOBufferSize());
			pLink->OnSessionReply(*pBuf,OriginalMsg);
			pBuf->ReturnBuffer();
		}	else
		{
			pLink->OnSessionSendCompleted(OriginalMsg);
...
*/
CISO8583IssuerGatewayFELink * CISO8583IssuerGatewayFEClientSession::GetLink()
{
	return	(CISO8583IssuerGatewayFELink *)m_LinkID.GetObject();
}

/*! see CMultiXSession::OnSessionKilled
in our case we just leave the session
*/
void	CISO8583IssuerGatewayFEClientSession::OnSessionKilled(CMultiXProcess *KillingProcess)
{
//	DebugPrint("Killed\n");
	Leave();
}
...
*/
bool	CISO8583IssuerGatewayFEClientSession::CheckStatus()
{
